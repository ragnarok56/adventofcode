...
sn = {'one': 1, 'two': 2, 'three': 3, 'four': 4, 'five': 5, 'six': 6, 'seven': 7, 'eight': 8, 'nine': 9}

def find_first_num(i: str):
    num = None
    idx = len(i) + 1
    for n_idx, c in enumerate(i):
        if c.isnumeric():
            num = int(c)
            idx = n_idx
            break
...
def find_last_num(i: str):
    # print(i)
    num = None
    idx = -1
    for n_idx, c in enumerate(i[::-1]):
        if c.isnumeric():
            num = int(c)
            idx = len(i) - n_idx
            break
...
def get_nums(i: str):
    yield find_first_num(i)
    yield find_last_num(i)

nums = []
for i in input_real.split('\n'):
    print(i)
    nx = list(get_nums(i))
    print(f'nums: {nx}')
    num = nx[0] * 10 + nx[1]
